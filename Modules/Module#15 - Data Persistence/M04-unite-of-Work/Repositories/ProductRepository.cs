...
namespace M04.CancellationTokenBenefit.Repositories;
...
public class ProductRepository(AppDbContext context) : IProductRepository
{
    ...
    public  void  AddProductAsync(Product product, CancellationToken ct = default)
    {
    ...
    {
        var product =  await context.Products.FirstOrDefaultAsync(p => p.Id == review.ProductId, ct) ?? throw new InvalidDataException();
        context.ProductReviews.Add(review);

        var reviews = await context.ProductReviews
                      .Where(r => r.ProductId == review.Id)
                      .ToListAsync(ct);
